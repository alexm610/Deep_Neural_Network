...
int main() {
	...
	for (y=0; y<120; y++) {
		for (x=0; x<160; x++) {
			// search for the first pixel that is TWO COLUMNS LEFT of the current coordinate
			colour = 0;
			pixel = 0;
			while (pixel <= num_pixels) {
				if (pixel_list[2*pixel+1] == y - 2) {
					break;
				}
				pixel++;
			}
			// we have found the portion of the list that is TWO COLUMNS LEFT OF THE CURRENT COORDINATE
			while (pixel_list[2*pixel+1] == y - 2) {
			...
			while (pixel_list[2*pixel+1] == y - 1) {
				if ((pixel_list[2*pixel] == x - 2) || (pixel_list[2*pixel] == x - 2)) {
					colour += 2;
				} else if ((pixel_list[2*pixel] == x - 1) || (pixel_list[2*pixel] == x - 1)) {
					colour += 4;
